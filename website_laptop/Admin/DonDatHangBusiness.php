...
class DonDatHangBusiness extends Data_Provider
{
    ...
    public function themMoi($donDatHang){
        ...
        $comm->bind_param("dddsdsd",$donDatHang->maLT,$donDatHang->soLuong,
            $donDatHang->thanhTien,$donDatHang->hTThanhToan,$donDatHang->xuLy,$donDatHang->ghiChu,$donDatHang->maKH);
        $ketQua = $comm->execute();
        $comm->close();
        $conn->close();
        if($ketQua){
    ...
    public function timKiemDonDatHang($maKH){
        $arrDonDatHang =Array();
        $conn = $this->ketNoi();
        $sqlTim ="Select * from DonDatHang where MaKH=".$maKH;
